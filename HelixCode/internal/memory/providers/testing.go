...
func (pts *ProviderTestSuite) TestIndexManagement(t *testing.T) {
	...
	// List indexes
	indexes, err := pts.provider.ListIndexes(pts.ctx, collectionName)
	if err != nil {
	...
	for _, index := range indexes {
		if index.Name == indexConfig.Name {
			found = true
			break
		}
	}
...
func (pts *ProviderTestSuite) generateTestVectors(count, dimension int) []*memory.VectorData {
...
func (pts *ProviderTestSuite) generateTestVector(dimension int) []float64 {
	return pts.generateTestVectorWithID(dimension, 0)
}

// generateTestVectorWithID generates a test vector with specific ID
func (pts *ProviderTestSuite) generateTestVectorWithID(dimension, id int) *memory.VectorData {
...
// TestProviderPerformance runs performance tests
func TestProviderPerformance(t *testing.T, provider VectorProvider, config map[string]interface{}) {
	suite := NewProviderTestSuite(provider, config)
	
	t.Run("BenchmarkStore", suite.BenchmarkStore)
	t.Run("BenchmarkSearch", suite.BenchmarkSearch)
}
