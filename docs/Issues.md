...
## HXC-279 — The gRPC identity check can be made to record a pass exactly when its own test fails, and nothing would notice
...
The check that confirms our remote-call service is genuinely the one answering now has a solid guard over HOW its decisive test is composed — it demands that two separate probes both hold — but nothing at all guards WHICH RESULT gets recorded once that composition has been evaluated. WHAT THIS MEANS IN PRACTICE: two small edits, each leaving the composition exactly as the guard requires, invert the meaning of the test entirely. The first negates the whole condition, so the check records a pass in precisely the situation where both probes failed. The second leaves the condition completely untouched and simply exchanges the two recording statements in the branches beneath it, which has the same inverting effect. In both cases the existing guard is satisfied, because what it certifies is genuinely still true — the condition really is a conjunction of both probes with no weakening — while the answer written down is the opposite of what was measured. WHY THE EXISTING GUARD CANNOT COVER IT: these are a different kind of promise. One is about how the question is asked, the other about how the answer is recorded, and stretching the first guard to cover the second would blur what either of them certifies. WHY NO TEST CATCHES IT TODAY: the four live-execution arms all reach their verdict before this comparison is exercised, and the one that would reach it deliberately removes the query tool from its environment, so none of them ever observes the recorded result. WHO IS AFFECTED: anyone relying on the automated gate to notice a wrong, foreign or half-started service before manual testing begins — an inverted check reports every healthy service as broken and, far worse, every broken one as healthy. HOW IT WAS FOUND: while verifying an unrelated widening, by asking whether any single edit could still defeat the test; the answer for composition was no, with an argument rather than a failed search, but asking the question one layer out surfaced this. Both edits were confirmed against the whole package with the query tool present, and nothing caught either. HOW TO REPRODUCE: negate the decisive condition as a whole, or exchange the two recording statements beneath it, then run the package and observe every check still passing. ACCEPTANCE CRITERIA: a guard asserts that the passing verdict is recorded on the branch taken when the condition holds and the failing verdict on the other, so that negating the condition or exchanging the branches is caught; it is proven real by paired mutations performing each of those two edits and confirming each makes the guard fail; and the existing composition guard keeps its own scope rather than absorbing this one.
