...
//
// THE DEFECT (reproduced by this file at RED_MODE=1): the mock LLM provider
...
// Any web page — on any site, opened by anyone who can route to this service —
// could therefore drive every route below and read the responses.
//
...
// (wildcard middleware) and MUST FAIL against fixed code. That falsifiability
// is the point — a guard that cannot fail proves nothing.
...
// allowlist comes from MOCK_LLM_ALLOWED_ORIGINS.
const (
...
// preflight (OPTIONS) path. Checking one and not the other is the exact
// sibling-miss that lets a hole survive a "fix".
func TestCORS_HostileOriginIsRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{"SimpleRequest_POST", http.MethodPost, "/v1/chat/completions"},
		{"SimpleRequest_GET", http.MethodGet, "/v1/models"},
		{"Preflight_OPTIONS", http.MethodOptions, "/v1/chat/completions"},
	}
...
func TestCORS_PermittedOriginIsAccepted(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"SimpleRequest", http.MethodPost, http.StatusOK},
		{"Preflight", http.MethodOptions, http.StatusNoContent},
	} {
		t.Run(tc.name, func(t *testing.T) {
			w := doRequest(t, r, tc.method, "/v1/chat/completions", permittedOrigin)
...
func TestCORS_NoOriginHeaderIsUnaffected(t *testing.T) {
	...
	w := doRequest(t, r, http.MethodPost, "/v1/chat/completions", "")
	...
	for _, allowed := range [][]string{
		...
		for _, origin := range []string{hostileOrigin, permittedOrigin, "*", ""} {
			for _, method := range []string{http.MethodGet, http.MethodPost, http.MethodOptions} {
				w := doRequest(t, r, method, "/v1/models", origin)
				if got := w.Header().Get("Access-Control-Allow-Origin"); got == "*" {
...
func TestCORS_PreservedCapabilities(t *testing.T) {
	...
	r := newCORSTestRouter(t, []string{permittedOrigin})
	w := doRequest(t, r, http.MethodOptions, "/v1/chat/completions", permittedOrigin)
