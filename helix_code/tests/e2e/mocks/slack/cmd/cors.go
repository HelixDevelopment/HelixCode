...
// request Origin at all. Any web page on any site — loaded by anyone able to
// route to this service — could therefore read back every message and webhook
// body the system under test had posted (GET /api/messages, GET /api/webhooks)
// and clear that captured state (DELETE on both).
//
...
//
// allowedOrigins comes from MOCK_SLACK_ALLOWED_ORIGINS via config.Load and
